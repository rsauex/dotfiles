  ...
  (package
    ...
    (source
     (origin
       ...
       (sha256
        (base32
         "1gbn98yhws2rmzzc5ijaxb41qi98zqg67z4gcihvsrd5xkq2npz5"))))
           ...
           (lambda* (#:key outputs #:allow-other-keys)
             (with-output-to-file "CMakeLists.txt"
               (lambda _
                 ...
                 (display "cmake_minimum_required(VERSION 3.10.0)\n")
...
(define-public kvantum-6
  (package
    ...
    (arguments
     `(#:tests? #f
       #:qtbase ,qt:qtbase-5
       #:configure-flags '("-DENABLE_QT5=OFF"
                           "-DENABLE_QT4=OFF"
                           "-DCMAKE_BUILD_TYPE=Release")
       #:phases
       (modify-phases %standard-phases
           ...
           (lambda* (#:key outputs #:allow-other-keys)
             (with-output-to-file "CMakeLists.txt"
               (lambda _
                 ...
                 (display "add_subdirectory(style)\n")))
             (substitute* `("style/CMakeLists.txt")
               (("\\$\\{_Qt6_PLUGIN_INSTALL_DIR\\}")
                (string-append (assoc-ref outputs "out") "/lib/qt6/plugins"))
               (("add_definitions\\(-DDATADIR=\"\\$\\{CMAKE_INSTALL_PREFIX\\}/share\"\\)")
                ;; TODO: Allow exprs in DATADIR (replace QLiteralString with QString)
                ;; "add_definitions(-DDATADIR=\"/run/current-system/profile/share\")"
                "add_definitions(-DDATADIR=\"/home/rsauex/.guix-home/profile/share\")"))
    ...
    (synopsis "SVG-based Qt6 theme engine plus a config tool and extra themes")
